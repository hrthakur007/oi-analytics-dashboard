...
// ATM Straddle & VWAP State
let straddleChartInstance = null;
let isAutoATM = true;
let fixedATMStrike = null;
window.straddleCache = {}; // Maps symbol -> array of { time, premium, vwap, spot, atmStrike }
window.lastDashData = null;
...
function updateDashboard(spotData, oiData, timeString) {
  ...
  // 6. Process ATM Straddle & VWAP Line Chart Data
  const targetStrike = isAutoATM ? atmStrike : (fixedATMStrike || atmStrike);
  const targetOI = oiData.find(item => Math.abs(item.strike_price - targetStrike) < 1) || oiData[closestIndex];
  
  if (targetOI) {
    const callLtp = targetOI.calls_ltp || 0;
    const putLtp  = targetOI.puts_ltp || 0;
    const premium = callLtp + putLtp;

    const callVwap = (targetOI.calls_average_price && targetOI.calls_average_price > 0) ? targetOI.calls_average_price : callLtp;
    const putVwap  = (targetOI.puts_average_price && targetOI.puts_average_price > 0) ? targetOI.puts_average_price : putLtp;
    const vwap     = callVwap + putVwap;

    const timeLabel = timeString || (new Date().toLocaleTimeString('en-IN', { hour: '2-digit', minute: '2-digit', hour12: false }));
    const symKey = currentSymbol.toLowerCase();

    if (!window.straddleCache[symKey]) {
      window.straddleCache[symKey] = [];
    }

    const cacheArr = window.straddleCache[symKey];
    const lastEntry = cacheArr.length > 0 ? cacheArr[cacheArr.length - 1] : null;

    if (lastEntry && lastEntry.time === timeLabel) {
      lastEntry.premium = premium;
      lastEntry.vwap = vwap;
      lastEntry.spot = ltp;
      lastEntry.atmStrike = targetStrike;
    } else {
      cacheArr.push({
        time: timeLabel,
        premium: premium,
        vwap: vwap,
        spot: ltp,
        atmStrike: targetStrike
      });
    }

    // Limit cache to 400 intraday points
    if (cacheArr.length > 400) cacheArr.shift();

    // Update Metrics Bar
    const atmBadge = document.getElementById('straddle-atm-badge');
    const premEl   = document.getElementById('straddle-premium-val');
    const vwapEl   = document.getElementById('straddle-vwap-val');
    const diffEl   = document.getElementById('straddle-diff-val');
    const spotEl   = document.getElementById('straddle-spot-val');
    const titleEl  = document.getElementById('straddle-chart-title');

    if (titleEl) titleEl.textContent = `${currentSymbol.toUpperCase()} ATM Straddle & VWAP`;
    if (atmBadge) atmBadge.textContent = `ATM: ${targetStrike}`;
    if (premEl) premEl.textContent = `₹${premium.toFixed(2)}`;
    if (vwapEl) vwapEl.textContent = `₹${vwap.toFixed(2)}`;
    if (spotEl) spotEl.textContent = `${ltp.toFixed(2)}`;

    if (diffEl) {
      const diff = premium - vwap;
      const diffPct = vwap > 0 ? (diff / vwap) * 100 : 0;
      const sign = diff >= 0 ? '+' : '';
      diffEl.textContent = `${sign}₹${diff.toFixed(2)} (${sign}${diffPct.toFixed(2)}%)`;
      diffEl.style.color = diff >= 0 ? '#80ffc2' : '#ff8080';
    }

    renderStraddleChart(cacheArr);
  }
...
// ============================================================
// ATM Straddle & VWAP Line Chart Engine
// ============================================================
function renderStraddleChart(dataPoints) {
  if (!dataPoints || dataPoints.length === 0) return;

  const labels   = dataPoints.map(d => d.time);
  const premiums = dataPoints.map(d => d.premium);
  const vwaps    = dataPoints.map(d => d.vwap);
  const spots    = dataPoints.map(d => d.spot);

  const canvas = document.getElementById('straddleChart');
  if (!canvas) return;
  const ctx = canvas.getContext('2d');

  if (straddleChartInstance) {
    straddleChartInstance.data.labels = labels;
    straddleChartInstance.data.datasets[0].data = premiums;
    straddleChartInstance.data.datasets[1].data = vwaps;
    straddleChartInstance.data.datasets[2].data = spots;
    straddleChartInstance.update();
    return;
  }

  // Create gradient fill for Straddle Premium
  const gradient = ctx.createLinearGradient(0, 0, 0, 300);
  gradient.addColorStop(0, 'rgba(56, 189, 248, 0.25)');
  gradient.addColorStop(1, 'rgba(56, 189, 248, 0.0)');

  straddleChartInstance = new Chart(ctx, {
    type: 'line',
    data: {
      labels: labels,
      datasets: [
...
// Handle Auto ATM / Fixed ATM Toggle Listeners
function setupStraddleToggleControls() {
  const btnAuto = document.getElementById('btn-straddle-auto');
  const btnFixed = document.getElementById('btn-straddle-fixed');
  ...
  if (btnAuto && btnFixed) {
    btnAuto.onclick = (e) => {
      ...
      btnAuto.style.background = 'var(--primary-color)';
      btnAuto.style.color = '#ffffff';
      btnFixed.style.background = 'transparent';
      btnFixed.style.color = 'var(--text-secondary)';
      if (window.lastDashData) {
        updateDashboard(window.lastDashData.spot, window.lastDashData.oi, window.lastDashData.time);
      }
    };
    ...
    btnFixed.onclick = (e) => {
      ...
      btnFixed.style.background = 'var(--primary-color)';
...
setupStraddleToggleControls();
if (document.readyState === 'loading') {
  document.addEventListener('DOMContentLoaded', setupStraddleToggleControls);
}
